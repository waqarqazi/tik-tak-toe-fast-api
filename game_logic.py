# tik-tak-toe-fast/game_logic.py
...
def check_winner(board):
    """Check if there is a winner or if the game is ongoing or drawn."""
...
def get_computer_move(board):
    """Finds the next available move for the computer (randomly chooses an empty spot)."""
    empty_cells = np.argwhere(board == 0)
    if len(empty_cells) == 0:
        raise HTTPException(status_code=400, detail="No available moves. The game is a draw.")

    move = empty_cells[random.choice(range(len(empty_cells)))]
...
def make_move(board, current_player):
    ...
    # Randomly get the computer move (for demonstration)
